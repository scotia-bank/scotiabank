...
export const EditContactView: React.FC<EditContactViewProps> = ({ isOpen, onClose, onSave, contact, theme = 'light', onDelete }) => {
        ...
        >
          <div className={`pt-12 pb-3 px-4 flex items-center border-b ${isDark ? 'border-white/5' : 'border-gray-100'}`}>
            ...
            <div className="flex-1 flex items-center justify-center">
              <h1 className="font-bold text-[17px]">Edit contact</h1>
            </div>
            <div className="w-8" />
          </div>
          ...
          <div className="flex-1 p-6 space-y-8">
            ...
            {onDelete && (
              <button 
                onClick={() => contact && onDelete(contact.id)}
                className="w-full py-4 text-red-500 font-bold border-2 border-red-500/20 rounded-xl mt-8 active:bg-red-500/10 transition-colors"
              >
                Delete contact
              </button>
            )}
          ...
          <div className="p-6 flex justify-end items-center">
            <span className="text-[#ED0711] font-bold text-lg mr-4">Save changes</span>
            <button 
